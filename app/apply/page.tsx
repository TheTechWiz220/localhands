  ...
  async function onFilesSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (const f of incoming) {
      ...
      try {
        ...
        // Accept if we got a data URL. Do not hard-block on canDisplayDataUrl —
        // Android/Google Photos often fail that check for valid JPG/PNG screenshots.
      ...
      <div className="rounded-xl border bg-white p-6 space-y-5">
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. 87 770 0011"
            className="w-full rounded-lg border px-3 py-2 text-sm"
          />
          <p className="text-xs text-gray-500 mt-1">
            9-digit Gambian number. Clients only see this after a job is confirmed.
          </p>
        </div>
            ...
            {selectedSkills
              ...
              .map((skill) => (
                <button
                  ...
                  type="button"
                  onClick={() => toggleSkill(skill)}
                  className="rounded-full border px-3 py-1 text-xs bg-green-600 text-white border-green-600"
                >
                  {skill}
                </button>
              ))}

    ...
    for (const f of incoming) {
      ...
      try {
        ...
        try {
          ...
          console.error("Proof preview failed:", err1, err2);
          skipped += 1;
      ...
      <div className="rounded-xl border bg-white p-6 space-y-5">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {SKILLS.map((skill) => (
              <button key={skill} type="button" onClick={() => toggleSkill(skill)} className={`rounded-full border px-3 py-1 text-xs ${selectedSkills.includes(skill) ? "bg-green-600 text-white border-green-600" : "bg-white text-gray-700"}`}>{skill}</button>
            ))}
            {selectedSkills.filter((s) => !(SKILLS as readonly string[]).includes(s)).map((skill) => (
              <button key={skill} type="button" onClick={() => toggleSkill(skill)} className="rounded-full border px-3 py-1 text-xs bg-green-600 text-white border-green-600">{skill}</button>
            ))}
          </div>
          <div className="flex gap-2">
            <input type="text" value={customSkill} onChange={(e) => setCustomSkill(e.target.value)} placeholder="Add another skill..." className="flex-1 rounded-lg border px-3 py-2 text-sm" onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addCustomSkill(); } }} />
        ...
        <div>
          ...
          <textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={3} placeholder="e.g. 5 years phone repair in Serrekunda. Available weekdays..." className="w-full rounded-lg border px-3 py-2 text-sm" />
        </div>

        <div>
          <label className="text-sm font-medium mb-1.5 block">Proof of work (photos)</label>
          <p className="text-xs text-gray-500 mb-2">Photos of jobs you have finished help us verify you faster. Up to 6 images, max 5MB each.</p>
          <input ref={fileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={onFilesSelected} />
          <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full border-2 border-dashed rounded-xl py-6 flex flex-col items-center gap-2 text-gray-500 hover:border-green-500 hover:text-green-700 transition-colors">
            ...
            <div className="grid grid-cols-3 gap-2 mt-3">
              {previews.map((src, i) => (
                <div key={`${i}-${src.slice(0, 24)}`} className="relative aspect-square">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={src} alt={`Preview ${i + 1}`} className="w-full h-full object-cover rounded-lg border bg-gray-100" onError={() => removeFile(i)} />
                  <button type="button" onClick={() => removeFile(i)} className="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full p-0.5">
        ...
        <label className="flex items-start gap-3 rounded-lg border p-3 cursor-pointer">
          <input type="checkbox" checked={confirmAdult} onChange={(e) => setConfirmAdult(e.target.checked)} className="mt-1" />
          <span className="text-sm text-gray-700">I confirm that I am <strong>18 years or older</strong>. LocalHands will review my national ID and complete an in-person assessment before approval.</span>
        </label>

        {error && (<p className="text-sm text-red-600 bg-red-50 rounded-lg px-3 py-2">{error}</p>)}

        <Button className="w-full" size="lg" disabled={loading || avatarUploading} onClick={submitApplication}>

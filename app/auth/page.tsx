...
export default function AuthPage() {
  ...
  async function sendOtp() {
    ...
    setLoading(false);
    if (error) {
      setError(error.message + " — Enable Phone auth + SMS provider in Supabase.");
    } else {
      setOtpSent(true);
  ...
  async function verifyOtp() {
    ...
    if (error) {
      ...
      // Workers go to application form; clients go to directory
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-10">
        ...
        {!otpSent ? (
          <>
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-400 mt-1">Gambia numbers (+220)</p>
            </div>
            {error && <p className="text-sm text-red-600">{error}</p>}
            {message && <p className="text-sm text-green-600">{message}</p>}
            <Button className="w-full" onClick={sendOtp} disabled={!phone || loading}>
      ...
      <p className="text-center text-xs text-gray-400 mt-6">
        Phone OTP needs SMS provider enabled in Supabase Auth settings.
      </p>
    </div>
  );
}
